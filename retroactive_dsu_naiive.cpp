...
class Persistent_DSU {
    ...
    void INSERT(Type t, int u, int v = -1, double time = 0.0) {
        if(t == Type::MAKE_SET) make_set(u);
        else if(t == Type::UNION) union_set(u, v, time);
        else assert(false);
    }
    void DELETE(double time) {
        ...
        if(t == Type::MAKE_SET) adj.erase(u);
        else if(t == Type::UNION) {
            adj[u][v].w.erase(adj[u][v].w.find(time));
            adj[v][u].w.erase(adj[v][u].w.find(time));
            if(adj[u][v].w.empty()) adj[u].erase(v);
            if(adj[v][u].w.empty()) adj[v].erase(u);
        }
        events.erase(it);
    }
    int QUERY(double time, int u, int v) {
        ...
        function<bool(int, double)> dfs = [&](int u, double tm) {
            if(u == v) return (tm <= time);
            vis[u] = true;
            for(auto [v, e] : adj[u]) {
                if(vis[v]) continue;
                for(auto t : e.w) {
                    if(dfs(v, max(tm, t))) return true;
                }
            }
            return false;
        };
